...
import pytest
import flint
from nuthatch.rings.polynomials import _Monomial, _Polynomial, Polynomial, PolynomialRing
...
class TestPolynomialDataClass:
    ...
    def test_mul(self):
        ...
        # (x+y)^2 == x^2 + 2xy + y^2
        assert self.p * self.p == _Polynomial(ZZ,{_Monomial((0,2)):flint.fmpz(1),_Monomial((0,1,1,1)):flint.fmpz(2),_Monomial((1,2)):flint.fmpz(1)})
...
class TestPolynomialRing:
    ...
    def test_gens_data(self):
        ...
        assert self.r.gens[0].data == _Polynomial(ZZ,{_Monomial((0,1)):flint.fmpz(1)})
